...
export default function AccountClient({ initialTab }: { initialTab?: Tab }) {
  ...
  // If user lands on /account?tab=profile (or similar), this prop will set the initial tab.
  // In case Next remounts the component with a different initialTab, keep it in sync.
  useEffect(() => {
  ...
  const handlePasswordChange = async (e: React.FormEvent) => {
    ...
    if (!validatePassword(passwordForm.new).valid) {
      setError('Password: 8+ chars with uppercase, lowercase, number');
      return;
    }
    setLoading(true);
    setError('');
          ...
          <div className="bg-white p-8 border scale-in">
            <div className="text-center mb-8">
              ...
              <p className="text-gray-500 text-sm">
                {mode === 'login' ? 'Sign in to your account' : 'Join the Delphine family'}
              </p>
            </div>
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <button type="submit" disabled={loading} className="btn-primary w-full">
                {loading ? (
                  <>
                    <span className="loader mr-2" /> Please wait...
                  </>
                ) : mode === 'login' ? (
                  'Sign In'
                ) : (
            ...
            <p className="text-center mt-6 text-sm text-gray-500">
              ...
              <button
                ...
                className="text-navy-900 ml-1 font-medium"
              >
                {mode === 'login' ? 'Sign up' : 'Sign in'}
              </button>
            </p>
            <div className="mt-6 p-4 bg-sand-50 text-sm text-gray-500">
              <p className="font-medium text-gray-700 mb-2">Demo Accounts:</p>
              <p>admin@delphine.com / admin123</p>
              <p>demo@delphine.com / demo123</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="bg-navy-900 text-white pt-32 pb-14">
        <div className="container-custom">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 fade-up">
            <div>
              <h1 className="heading-2 mb-1">Hello, {user?.name?.split(' ')[0]}</h1>
              <p className="text-white/60">{user?.email}</p>
            </div>
            <div className="flex gap-3">
              {user?.isAdmin && (
                <Link href="/admin" className="btn-white text-sm">
                  Admin Dashboard
                </Link>
              )}
              <button
                onClick={logout}
                className="btn border border-white text-white hover:bg-white hover:text-navy-900 text-sm"
              >
                Sign Out
              </button>
            </div>
          ...
          {tab === 'orders' &&
            (orders.length > 0 ? (
              <div className="space-y-4 fade-up">
                {orders.map((order) => (
                  <div key={order.id} className="bg-white border p-5 hover:shadow-md transition-shadow">
              ...
              <div className="text-center py-16 bg-sand-50 fade-up">
                <Icons.bag className="w-12 h-12 text-gray-200 mx-auto mb-4" />
          ...
          {tab === 'profile' && (
            <div className="max-w-md fade-up">
              <form onSubmit={handleProfileUpdate} className="space-y-4">
                ...
                {success && (
                  <div className="p-3 bg-green-50 border border-green-200 text-green-600 text-sm">{success}</div>
                )}
          ...
          {tab === 'security' && (
            <div className="max-w-md fade-up">
              ...
              <form onSubmit={handlePasswordChange} className="space-y-4">
                ...
                <div>
                  <label className="text-xs text-gray-500 uppercase tracking-wide mb-1 block">Confirm New Password</label>
                  <input
                    type="password"
